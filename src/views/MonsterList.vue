<template>
  <DataTable :loading="monsterStore.loading" scrollable scrollHeight="600px" :value="filteredMonsters"
    v-model:filters="filters" :globalFilterFields="['nom', 'nom_normal', 'zone', 'sous_zone']"
    :virtualScrollerOptions="{ itemSize: 50 }">
    <template #header>
      <div class="flex justify-end">
        <IconField>
          <InputText v-model="filters['global'].value" placeholder="Recherche de monstre" />
          <SelectButton v-model="captureStatus" :options="options" optionLabel="name" optionValue="value" />
        </IconField>
      </div>
    </template>
    ...
    <Column :showFilterMenu="true" field="nom" header="Nom">
      <template #body="slotProps">
        <div style="display: flex; flex-direction: row; gap: 8px;">
          <img style="background-color: #F5ECDD;" :src="slotProps.data.image_url" :alt="slotProps.data.image"
            height="50" />
          <div>
            <div><strong>{{ slotProps.data.nom }}</strong></div>
    ...
    <Column field="quantite" header="Quantité">
      <template #body="slotProps">
        <div style="display: flex; align-items: center; gap: 8px;">
          <Button size="small" icon="pi pi-minus" severity="danger"
          ...
          <Button size="small" icon="pi pi-plus" @click="onIncreaseQty(slotProps.data.id, slotProps.data.quantite)" />
...
const nbNotCaptured = computed(() => {
  return monsterStore.monsters.filter((monster: MonsterTrade) => monster.type === props.monsterType && monster.quantite === 0).length;
});
...
const nbAll = computed(() => {
  return monsterStore.monsters.filter((monster: MonsterTrade) => monster.type === props.monsterType).length;
});
...
const options = computed(() => {
  return [
    ...
    {
      name: `Capturé (${nbCaptured.value})`, value: 2
    },
